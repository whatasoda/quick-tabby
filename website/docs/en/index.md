---
pageType: home
hero:
  name: QuickTabby
  text: Fast Tab Switching
  tagline: Navigate tabs sorted by recently used with keyboard shortcuts
  image:
    src:
      dark: /screenshots/dark-main.png
      light: /screenshots/light-main.png
    alt: QuickTabby tab switcher interface
  actions:
    - theme: brand
      text: Learn More
      link: /en/features
    - theme: alt
      text: Install
      link: https://chromewebstore.google.com/detail/quicktabby/gpljmkkeodeoedpcdabdmcnlaljpdhni
features:
  - title: Recently Used Order
    details: Tabs are automatically sorted by recent usage for quick switching
    icon: 🕐
  - title: Keyboard First
    details: Navigate and switch tabs entirely with keyboard shortcuts
    icon: ⌨️
  - title: Lightweight
    details: Fast and minimal resource usage
    icon: ⚡
---
